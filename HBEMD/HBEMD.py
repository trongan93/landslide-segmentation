...
class HBEMD():
    def __init__(self, img_path):
        rgb_img_raw = cv2.resize(cv2.cvtColor(cv2.imread(img_path),cv2.COLOR_BGR2RGB), (224,224))
        self.rgb = image_condition_adjustment(rgb_img_raw, r_added_value=0, g_added_value=0, b_added_value=0)

    def hue_value(self):
        hsvInputImage_Full = cv2.cvtColor(self.rgb.astype(np.float32), cv2.COLOR_RGB2HSV_FULL)
    ...
    def sifting(self, hue_img, max_imf_value):
        ...
        for i in range(0, imfs_no):
            ...
            imf_hue_degree = change_negative_hue_to_positive(imf_hue_degree_old)
